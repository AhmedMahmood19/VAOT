...
# def _get_kendalls_tau(embs_list, mask_list, split, kt_dist, visualize=False):
#     """Get nearest neighbors in embedding space and calculate Kendall's Tau."""
#     num_seqs = len(embs_list)
#     taus = np.zeros((num_seqs * (num_seqs - 1),))
#     idx = 0
...
#         query_mask = mask_list[i]  # Corresponding mask for query_feats
        
#         for j in range(num_seqs):
#             if i == j:
#                 continue
            
#             candidate_feats = embs_list[j]
#             candidate_mask = mask_list[j]  # Corresponding mask for candidate_feats
            
#             # Apply masks to filter out invalid frames
#             query_feats = query_feats[query_mask]
#             candidate_feats = candidate_feats[candidate_mask]
            
#             dists = cdist(query_feats, candidate_feats, kt_dist)
